...
class Investment(models.Model):
    ...
    def calculate_profit(self):
        """Calculate a 2% profit based on the investment amount."""
        return self.amount * Decimal('0.02')  # Use Decimal instead of float

        super().save(*args, **kwargs)  # Call the parent class save method

    def __str__(self):
        return f"{self.user.username} - {self.amount} - {self.profit} - {self.status}"
